...
// ── 핫픽스 데이터 저장소 ─────────────────────────────────────────────────────
// 코드 실행(patchCode)은 보안상 불허. 선언적 데이터만 허용.
// JSON 스키마:
//   translations   : Record<string,string>  텍스트 교체
//   config         : Record<string,unknown>  게임 상수 오버라이드 (TURN_MS 등)
//   featureFlags   : Record<string,boolean>  기능 on/off 스위치
//   imageOverrides : Record<string,string>   이미지 경로 → 새 URL
//   soundOverrides : Record<string,string>   사운드 경로 → 새 URL
//   extraEvents    : ActiveEvent[]           이벤트 풀에 추가할 새 이벤트
//   endingPatches  : Record<string,Partial<...>>  엔딩 텍스트/조건 일부 덮어쓰기
const HOTFIX_DATA: {
...
/** 이미지 경로를 핫픽스 오버라이드로 교체합니다. */
function hfImage(path: string): string {
  return (HOTFIX_DATA.imageOverrides[path] as string | undefined) ?? path
}

/** 사운드 경로를 핫픽스 오버라이드로 교체합니다. */
function hfSound(path: string): string {
  return (HOTFIX_DATA.soundOverrides[path] as string | undefined) ?? path
}

/** 게임 상수를 핫픽스 값으로 읽습니다. (예: hfConfig('TURN_MS', 100)) */
export function hfConfig<T>(key: string, defaultValue: T): T {
  const v = HOTFIX_DATA.config[key]
...
const TRANSLATIONS: Record<string, string> = {
  ...
  '한번 더 돌리기': 'Spin Again',
  '재배 시작': 'Start Growing',
  '슬롯 전 선택': 'Before the Slot',
  '광고를 보면 재롤 기회를 한 번 더 받아요!': 'Watch an ad for one extra reroll!',
  '전광고 보기': 'Watch Full Ad',
  '재롤 +1 보너스': '+1 Reroll',
  '그냥 시작': 'Skip',
  '광고 없이 진행': 'Play without ad',
  '잭팟!': 'Jackpot! ',
...
function buildActionFrames(stage: number, action: ActionKind): string[] {
  ...
  let frames: string[]
  if (action === 'eat') frames = [`${base}_eat1.png`, `${base}_eat2.png`]
  else if (action === 'drink') frames = [`${base}_drink1.png`, `${base}_drink2.png`]
  else if (action === 'dgdg') frames = [`${base}_dgdg1.png`, `${base}_dgdg2.png`, `${base}_dgdg3.png`]
  else if (action === 'exercise') frames = [`${base}_ex1.png`, `${base}_ex2.png`]
  else if (action === 'solar') frames = imgStage === 1 ? [`${base}_solar.png`] : [`${base}_solar1.png`, `${base}_solar2.png`]
  else frames = [`${base}_normal.png`]

  return frames.map(hfImage)
}

function playSound(path: string, volume = 0.82): void {
  if ((window as any).gameAudioMuted) return
  const resolvedPath = hfSound(path)
  const cached = soundCache.get(resolvedPath)
  const audio = cached ? (cached.cloneNode() as HTMLAudioElement) : new Audio(resolvedPath)
  audio.volume = volume
...
// 엔딩마다 배경 일러스트 위치를 좌/우로 다양하게 배치 (정중앙 고정 X)
function getEndingBgPosition(seed: number): string {
  const positions = [
    '20% 36%', // 좌측
    '80% 40%', // 우측
    '30% 32%', // 좌측 위
  ...
  return positions[(Math.max(1, seed) - 1) % positions.length]
}
...
function pickEnding(state: GameState): EndingResult {
  // 1. Cloud Ending (E32) - 능력치가 하나라도 0 이하(바닥)이면 구름 엔딩 (음수/NaN 방어 포함)
    ...
    return (
      ...
      >
        ...
        <div className="harvest-card" style={{ cursor: 'default' }}>
          <div style={{ width: '100%', padding: '0 10px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <h3 style={{
  ...
  return (
    <div className="ending-overlay" role="dialog" aria-modal="true" aria-label={t('엔딩 결과')}>
      ...
      <div className="ending-overlay-wrapper">
        <div
          className={`collection-detail-card scrapbook-ending-card ${getPastelPatternClass(ending.imageIndex)}`}
          style={{
            '--ending-img': `url(${getEndingIconPath(ending)})`,
            '--ending-bg-pos': getEndingBgPosition(ending.imageIndex),
          } as React.CSSProperties}
        >
      ...
      {selectedEnding && (
        <div className="collection-detail-overlay" onClick={() => setSelectedEnding(null)}>
          <div className="collection-detail-wrapper" onClick={(e) => e.stopPropagation()}>
            <div
              className={`collection-detail-card scrapbook-ending-card ${getPastelPatternClass(selectedEnding.imageIndex)}`}
              style={{
                '--ending-img': `url(${getEndingIconPath(selectedEnding)})`,
                '--ending-bg-pos': getEndingBgPosition(selectedEnding.imageIndex),
              } as React.CSSProperties}
            >
...
function App() {
    ...
    const checkHotfix = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.translations)   { Object.assign(TRANSLATIONS, data.translations);               console.log('[Hotfix] translations:', Object.keys(data.translations).length, 'keys') }
          if (data.config)         { Object.assign(HOTFIX_DATA.config, data.config);                console.log('[Hotfix] config:', data.config) }
          if (data.featureFlags)   { Object.assign(HOTFIX_DATA.featureFlags, data.featureFlags);    console.log('[Hotfix] featureFlags:', data.featureFlags) }
          if (data.imageOverrides) { Object.assign(HOTFIX_DATA.imageOverrides, data.imageOverrides);console.log('[Hotfix] imageOverrides:', Object.keys(data.imageOverrides).length) }
          if (data.soundOverrides) { Object.assign(HOTFIX_DATA.soundOverrides, data.soundOverrides);console.log('[Hotfix] soundOverrides:', Object.keys(data.soundOverrides).length) }
          if (Array.isArray(data.extraEvents) && data.extraEvents.length > 0) {
            EVENT_POOL.push(...(data.extraEvents as typeof EVENT_POOL))
            console.log('[Hotfix] extraEvents added:', data.extraEvents.length)
          }

          // 보안: 원격 코드(patchCode) 실행은 제거되었습니다.
          // 서버 계정이 탈취될 경우 모든 기기에서 임의 코드가 실행될 수 있어,
          // 선언적 데이터(선언형 객체 교체)만 허용합니다.
  ...
  const playRewardedAd = async (): Promise<boolean> => {
    if (import.meta.env.VITE_SKIP_ADS === 'true') {
      return true
    }
    if (!isNative) {
  ...
  const potatoImage = game.actionPlayback
    ? hfImage(game.actionPlayback.frames[game.actionPlayback.frameIndex])
    : hfImage(`/assets/original/potato${growthStage}_normal.png`)
            ...
            >
              <div
                style={{
                  ...
                  padding: '22px 24px', maxWidth: '300px', width: '88%', textAlign: 'center',
                  fontFamily: 'Gaegu, cursive', color: '#4a2814',
                }}
              >
                <div style={{ fontSize: '28px', marginBottom: '8px' }}>🎬</div>
                <p style={{ fontSize: '17px', fontWeight: 700, margin: '0 0 4px', whiteSpace: 'pre-line', lineHeight: 1.55 }}>
                  {lang === 'ko'
                    ? '지난 회차 엔딩을 아직 확인하지 않았어요.'
                    : "You haven't seen the last ending yet."}
                </p>
                <p style={{ fontSize: '14px', margin: '0 0 18px', color: '#6b4226', lineHeight: 1.5 }}>
                  {lang === 'ko'
                    ? '광고를 보면 지금 바로 엔딩을 볼 수 있어요.'
                    : 'Watch a short ad to view the ending now.'}
                </p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <SlideToWatchAd
                    label={lang === 'ko' ? '밀어서 광고 보고 엔딩 보기' : 'Slide to Watch Ad & See Ending'}
                    onConfirm={async () => {
                      ...
                      try {
                        ...
                        console.error('Ending alert ad error:', err)
                        success = false
                      } finally {
                  ...
                  <button
                    ...
                    style={{
                      ...
                      padding: '9px 20px', cursor: 'pointer', color: '#4a2814', width: '100%',
                    }}
                  >
                    🌱 {lang === 'ko' ? '그냥 새로 시작' : 'Start Over'}
                  </button>
                </div>
              </div>
            </div>
          ...
          {adNotice && (
            ...
            >
              <div
                style={{
                  background: '#fffbe9', border: '4px solid #6b4a2b', borderRadius: '18px',
                  padding: '22px 26px', maxWidth: '320px', width: '88%', textAlign: 'center',
                  fontFamily: 'Gaegu, cursive', color: '#4a2814',
                }}
                onClick={(e) => e.stopPropagation()}
              >
                <div style={{ fontSize: '30px', marginBottom: '8px' }}>📺</div>
                <p style={{ fontSize: '18px', fontWeight: 700, margin: '0 0 18px', whiteSpace: 'pre-line', lineHeight: 1.55 }}>{adNotice}</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center' }}>
                  <SlideToWatchAd
                    label={t('밀어서 광고 보고 엔딩 보기')}
                    onConfirm={async () => {
                      setAdNotice(null)
                      if (adInFlightRef.current) return
                      adInFlightRef.current = true
                      ...
                      try {
                        ...
                        console.error('Rewarded ad retry error:', err)
                        success = false
                      } finally {
                        setIsAdLoading(false)
                        adInFlightRef.current = false
                  ...
                  <button
                    type="button"
                    onClick={() => { setAdNotice(null); startNewRun() }}
                    style={{
                      fontFamily: 'inherit', fontSize: '15px', fontWeight: 700,
                      background: '#efebe9', border: '2px solid #6b4a2b', borderRadius: '12px',
                      padding: '8px 20px', cursor: 'pointer', color: '#4a2814', width: '100%',
                    }}
                  >
                    🌱 {t('새로 키우기')}
                  </button>
